...
#include "TopasShots/widgets/roi_selector.h"
#include "TopasShots/viewers/image_viewer.h"
...
QString MainWindow::buildFreeSnipFileName(const QString &comment) const
{
    ...
    if (marker.isEmpty())
        return QStringLiteral("TopasShots_free-snip_%1.JPG").arg(timestamp);

    return QStringLiteral("TopasShots_free-snip_%1_%2.JPG").arg(timestamp, marker);
}
...
void MainWindow::updateConfigurationErrors()
{
    ...
    setLineEditError(ui_->le_campaign_, tshots::core::PathTokens::requiredPathToken(settings.campaign).isEmpty());
...
QString MainWindow::requestOptionalComment(const QString &title, const QString &label)
{
    ...
    return tshots::core::PathTokens::markerToken(text);
}
...
void MainWindow::updateCurrentSectionUi()
{
    ...
    if (controller_.settings().sequence.currentKind == tshots::core::SectionKind::Transition)
    {
...
{
    const auto &target = controller_.settings().captureTarget;
    QScreen *screen = tshots::capture::ScreenshotService::screenForTarget(target);
    ...
    if (target.isWindow())
    {
        ...
        allowedSelectionGlobal = tshots::capture::CaptureTargetEnumerator::windowRect(target.windowId);
    ...
    connect(selector, &RoiSelector::roiSelected, this, [this](const QRect &globalRect) {
        ...
        if (target.isWindow())
        {
            const QRect targetRect = tshots::capture::CaptureTargetEnumerator::windowRect(target.windowId);
...
bool MainWindow::takeShot(bool manual, const QString &marker, bool askManualComment)
{
    ...
    if (manual)
    {
        ...
        QString error;
        if (!controller_.saveCampaignShotPair(tshots::core::ShotSource::Manual,
                                              comment,
                                              manual_shot_full_,
                                              manual_shot_crop_,
                                              &paths,
    ...
    else
    {
        ...
        tshots::core::SavedShotPaths paths;
        QString error;
        if (!controller_.saveCampaignShotPair(tshots::core::ShotSource::Auto,
                                              marker,
                                              auto_shot_full_,
                                              auto_shot_crop_,
                                              &paths,
...
QString MainWindow::currentSectionIdToken() const
{
    ...
    return tshots::core::PathTokens::requiredPathToken(sectionId);
}
...
QDir MainWindow::currentAutoShotDir(const QString &frameType) const
{
    ...
    const QString campaign = tshots::core::PathTokens::requiredPathToken(settings.campaign);
...
{
    const QString markerToken = tshots::core::PathTokens::markerToken(marker).toLower();
