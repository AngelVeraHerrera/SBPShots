# SBP Shots — User Manual

SBP Shots is a small Windows utility that takes structured, timestamped screenshots of
a **sub-bottom profiler** display (or any other window/screen) while an
...
| Option | What it does |
|---|---|
| **Always on Top** | Keeps the SBP Shots window above all others. |
| **Hide on Capture** | Hides the SBP Shots window for a moment while the shot is taken, so it never appears in the image. |
...
When automatic capture runs, SBP Shots adds helpful markers automatically:
