...
public:
    ...
    const sbpshots::core::ApplicationSettings &settings() const;

    /** @brief Returns a mutable reference to the current application settings. */
    sbpshots::core::ApplicationSettings &settings();
     ...
     */
    QList<sbpshots::capture::CaptureTarget> enumerateTargets(quintptr selfWindowId) const;

    /**
     * @brief Sets a new capture target and clears the current ROI.
     * @param target New capture target.
     */
    void setCaptureTarget(const sbpshots::capture::CaptureTarget &target);
     ...
     */
    bool saveCampaignShotPair(sbpshots::core::ShotSource source,
...
private:
    ...
    sbpshots::core::ApplicationSettings settings_;
};
