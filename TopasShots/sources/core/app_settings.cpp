...
ApplicationSettings AppSettingsStore::load()
{
    ...
    app.captureTarget.type = targetType == "window"
        ? tshots::capture::CaptureTargetType::Window
        : tshots::capture::CaptureTargetType::Screen;
    ...
    if (app.captureTarget.type == tshots::capture::CaptureTargetType::Window)
    {
        tshots::capture::CaptureTarget resolved;
        if (tshots::capture::CaptureTargetEnumerator::findWindowByTitle(app.captureTarget.persistentName, &resolved))
        {
            app.captureTarget = resolved;
        }
        else
        {
            app.captureTarget = tshots::capture::CaptureTargetEnumerator::primaryScreenTarget();
        }
    }
    else
    {
        const QList<tshots::capture::CaptureTarget> targets =
            tshots::capture::CaptureTargetEnumerator::enumerateTargets();

        bool found = false;
        for (const auto &target : targets)
        {
            if (target.type == tshots::capture::CaptureTargetType::Screen
...
void AppSettingsStore::save(const ApplicationSettings &settingsData)
{
    ...
    settings.setValue("CaptureTarget/type",
                      settingsData.captureTarget.type == tshots::capture::CaptureTargetType::Window
                          ? "window"
                          : "screen");
